...
document.addEventListener('DOMContentLoaded', function(){
  ...
  setWidth();
  topLayer.style.width= window.innerWidth/2 -wrapper.offsetLeft + skew + 'px';    
  
  wrapper.addEventListener('mousemove', function(e){
    delta = (e.clientX - window.innerWidth / 2) *0.3;  //na srodku=0, im dalej od środka tym bardziej popycha "belkę" oddzielającą warstwy
...
window.addEventListener('resize', function(){
  ...
  handle.style.left = window.innerWidth / 2 -wrapper.offsetLeft + 'px';
  topLayer.style.width= window.innerWidth / 2 -wrapper.offsetLeft + skew + 'px';
});